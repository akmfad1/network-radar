...
- 🔄 **به‌روزرسانی خودکار** - پیش‌فرض بررسی: هر 30 ثانیه (قابل تنظیم در `config.yaml` با `check_interval`)
...
پیش‌فرض‌های رایج در `agent_config.yaml`:
```yaml
check_interval: 60
concurrency: 4
batch_send_interval: 10
api_key: "<set-to-match-server>"
```
...
- `web_port`: پورت وب‌سرور (پیش‌فرض `5000`).
- `check_interval`: فاصله زمانی بررسی‌ها در سرور/ایجنت‌ها (پیش‌فرض `30` ثانیه).
- `retention_hours`: نگهداری رکوردها (پیش‌فرض `24` ساعت).
- `agent_id`: شناسه محلی ایجنت (پیش‌فرض `"display"`).
- `api_key`: کلید احراز هویت برای ایجنت‌ها (پیش‌فرض `"change-me"`؛ لطفاً آن را تغییر دهید).
